  ...
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="background:#1a1a1a;border-radius:16px;overflow:hidden;max-width:600px;width:100%;">
      ...
      <tr><td style="background:linear-gradient(135deg,#1a0800,#2d1100);padding:32px 40px;text-align:center;">
        ...
        <p style="margin:6px 0 0;color:#888;font-size:13px;">Plan de Entrenamiento</p>
      </td></tr>

      <tr><td style="padding:36px 40px;">
        @if($action === 'add')
          <h2 style="color:#fff;font-size:18px;font-weight:700;margin:0 0 8px;">💪 Nuevo Ejercicio Asignado</h2>
          <p style="color:#aaa;font-size:14px;margin:0 0 24px;">Hola <strong style="color:#fff;">{{ $user->name }}</strong>, tu entrenador ha añadido un nuevo ejercicio a tu rutina:</p>
        ...
        @elseif($action === 'delete')
          <h2 style="color:#fff;font-size:18px;font-weight:700;margin:0 0 8px;">🗑️ Ejercicio Eliminado</h2>
          <p style="color:#aaa;font-size:14px;margin:0 0 24px;">Hola <strong style="color:#fff;">{{ $user->name }}</strong>, un ejercicio ha sido eliminado de tu rutina de entrenamiento.<br>@if($details)<br><strong style="color:#ccc;">Ejercicio eliminado:</strong><br>{!! $details !!}@endif</p>
        @endif

        <a href="{{ config('app.url') }}/dashboard/entrenamiento"
      ...
      <tr><td style="background:#111;padding:20px 40px;text-align:center;border-top:1px solid #222;">
        <p style="color:#555;font-size:11px;margin:0;">© {{ date('Y') }} Tone Trainer. Correo automático.</p>
      </td></tr>
    </table>
  </td></tr>
</table>
